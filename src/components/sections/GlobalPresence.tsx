      ...
      <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {content.offices.map((office) => (
          <Card key={office.id}>
            ...
            <h3 className="mt-4 text-sm font-semibold text-white">{office.country}</h3>
            <p className="mt-1 text-xs text-white/60">{office.entity}</p>
            <p className="mt-2 text-xs font-medium text-sky-400">{office.role}</p>
            {office.address && <p className="mt-3 text-xs leading-relaxed text-white/50">{office.address}</p>}
          </Card>
        ))}
      </div>
    </SectionWrapper>
