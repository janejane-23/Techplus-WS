        ...
        {content.items.map((item) => {
          ...
          return (
            <Card key={item.id}>
              ...
              <h3 className="mt-4 text-lg font-semibold text-slate-900">{item.painPoint}</h3>
              <p className="mt-2 text-sm leading-relaxed text-slate-600">{item.howTechPlusHelps}</p>
            </Card>
          );
        })}
      </div>
